...
The plugin runs inside the Paperclip board UI and ships with a complete template library: 15 presets, 27 modules, 16 optional roles (CEO is the only base role).
...
├── modules/                   # Composable capabilities (27 modules)
│   └── <module>/
