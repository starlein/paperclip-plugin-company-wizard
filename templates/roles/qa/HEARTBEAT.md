...
- **PR-review mode:** standard PR review can name QA as an executionPolicy stage; record `approved`/`changes_requested` only when you are its active participant, and let Paperclip route the next action. If `docs/lean-delivery.md` exists, QA instead provides bounded evidence and is not a default stage. Never mark the originating implementation issue `done`, never act on the Code Reviewer's stage, and never create a QA courier issue. For lean/advisory evidence, post exact-head pass/fail evidence and return the same issue directly to the implementation owner in the same heartbeat, with exact reproduction steps on failure; the implementation owner opens the merge gate after all triggered evidence returns.
