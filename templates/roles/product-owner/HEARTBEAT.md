...
- For an assigned acceptance handoff, inspect its complete action path before acting: current `executionPolicy` participant, `GET /api/issues/{id}/interactions`, `GET /api/issues/{id}/approvals`, user owner, monitor, wake, and recovery state. Do not override a pending interaction or approval merely because `executionPolicy` is null. Advance your own active stage with its documented verdict; for an advisory handoff, record acceptance evidence and return the same issue to the implementation owner. Never mark the originating implementation issue `done` while its PR is open or another review/merge stage remains. A standalone acceptance deliverable can finish when its own criteria are satisfied.
