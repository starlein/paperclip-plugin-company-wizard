...
- Make workspace intent explicit at creation; never rely on implicit inheritance. Top-level issues and subissues set `"executionWorkspaceSettings": { "mode": "isolated_workspace" }` by default. `parentId` expresses task hierarchy, not consent to share a checkout. Reuse is exceptional and explicit via `inheritExecutionWorkspaceFromIssueId` only when the task requires the same code change. Keep `projectId` explicit on both.
