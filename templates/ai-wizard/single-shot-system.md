...
`lean-delivery` is opt-in: include it only when the user explicitly requests lean delivery or its single-gate/WIP policy. Otherwise leave it unselected. When selected, include its `pr-review` and `github-repo` dependencies.
