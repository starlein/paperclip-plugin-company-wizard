...
**0.6.4 update (2026-09-16).** Adds explicit, guarded sync for an empty operator-configured template directory and emits Codex-compatible `name` and `description` YAML frontmatter for managed Company Skills. The skill slug remains the stored Paperclip name because Paperclip derives it from the file and rewrites it on rename; the human-readable name remains in generated workspace instructions. Existing Company Skills require a refresh to gain frontmatter. This release does not make a new host compatibility claim.
